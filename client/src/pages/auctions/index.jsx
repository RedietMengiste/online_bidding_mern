...
import {fetchAllAuctionsSuccessAsync, fetchAuctionsByOwnerSuccessAsync, fetchAuctionSuccessAsync} from "../../store/auction/action"
...
export const AuctionPage = () => {
    ...
    const { auctions, page, limit, total, fetchAuctionsLoading } = useSelector((state)=>state.auction)
             ...
             {/* first three auctions */}
                ...
                <section className={"featured-auction-section padding-bottom mt--240 mt-lg--440 pos-rel"}>
                    <div className={"container"}>
                        ...
                        {
                            auctions.slice(auctions.length > 3 ? auctions.length - 3:auctions.length).map((auction)=>{
                               return <div className={"col-sm-10 col-md-6 col-lg-4"}>
                                    <div className={"auction-item-2"}>
                                            <div className={"auction-thumb"}>
                                                <Link ><img src={`${process.env.REACT_APP_AUCTION_IMG_URL}/${auction.image}`} alt={auction.itemName}/></Link>
                                            </div>
                                            <div className={"auction-content"}>
                                                <h6 className={"title"}>
                                                    <a href="#0">{auction.itemName} {auctionState(auction)}</a> 
                                                </h6>
                                                <div className={"bid-area"}>
                                                    <div className={"bid-amount"}>
                                                        ...
                                                        <div className={"amount-content"}>
                                                            <div className={"current"}>{auction.description}</div>
                                                            <div className={"amount"}>{auction.createdAt}</div>
                                                        </div>
                                                    </div>
                                                ...
                                                <div className={"countdown-area"}>
                                                    ...
                                                    <Link>
                                                        <span className={"total-bids"}>Shop - {  auction.shop.name}</span>
                                                    </Link>
                                                </div>
                                                <div className={"text-center"}>
                                                    <Link to='/auctions' className={"custom-button"}>See Auctions</Link>
                                                </div>
                                            </div>
                                        </div>
                                </div>
                    ...
                    <div className={"row mb-30-none justify-content-center"}>
                        {
                            auctions.map((auction)=>{
                               return <div className={"col-sm-10 col-md-6 col-lg-4"}>
                                    <div className={"auction-item-2"}>
                                            <div className={"auction-thumb"}>
                                                <Link ><img src={`${process.env.REACT_APP_AUCTION_IMG_URL}/${auction.image}`} alt={auction.name}/></Link>
                                            </div>
                                            <div className={"auction-content"}>
                                                <h6 className={"title"}>
                                                ...
                                                <div className={"text-center"}>
                                                    <a href="#0" className={"custom-button"}>See Auctions</a>
                                                </div>
                                            </div>
                                        </div>
                                </div>
