...
export const fetchAuctionStart = () => ({
    type: AuctionActionTypes.AUCTION_FETCH_ALL_START
});

export const fetchAuctionSuccess = (auction) => ({
    type: AuctionActionTypes.AUCTION_FETCH_ALL_SUCCESS,
    payload:{
        auction,
    },
});
...
export const fetchAuctionError = (error) => ({
    type: AuctionActionTypes.AUCTION_FETCH_ALL_ERROR,
    payload:{
      error,
    },
});
 ...
 * @returns 
 */
export const createAuctionSuccessAsync = (auction, shopId) => {
    return async (dispatch,getState)=> {
        ...
        try{
            ...
            const response=await axios.post(
                `${process.env.REACT_APP_API_URL}/auctions/by/${shopId}`,auction,{
                    headers:{
    ...
    return async (dispatch,getState)=> {
        ...
        try{
            ...
            const response=await axios.put(
                `${process.env.REACT_APP_API_URL}/auction/${id}`,form,{
                    headers:{
                        Authorization:`Bearer ${token}`,
                    },
                }
...
export const fetchAllAuctionsSuccessAsync = (page,limit) =>{
    return async(dispatch, getState) => {
        ...
        try{
            ...
            dispatch(fetchAllAuctionsSuccess(response.data.result.docs,response.data.result.page,response.data.result.limit,response.data.result.total))
        }
        catch(error){
...
export const fetchAuctionsByUserSuccessAsync = (page,limit, shopId) =>{
    return async(dispatch, getState) => {
        const{
            user:{token},
        }=getState();
        try{
            dispatch(fetchAllAuctionsStart());
            const response = await axios.get(
                `${process.env.REACT_APP_API_URL}/auctions/by/${shopId}`,
                {
                    params:{
                        page,
                        limit,
